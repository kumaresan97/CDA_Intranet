...
const DepartmentHeader = ({ icon, title, subtitle, loading }: any) => {
    ...
    const SkeletonLoader = () => {
        ...
        return (
            <div style={{
                ...
                <Skeleton.Input style={{ width: 300, }} size="default"
                    active />
    ...
    return loading ? SkeletonLoader() : (
        <section className="department-header">
            {icon && (
                <img src={icon} alt={title} className="dept-icon" />
            )}

            <h1 className="dept-title" style={{
                textAlign: "center"
            }}>{title}</h1>
