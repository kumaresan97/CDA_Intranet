...
import { getChoiceDropdownOptions, ServiceTaskService } from '../../../../Services/Department/DepartmentService';
...
const defaultimg: any = require("../../../assets/images/Service.png")
...
const DepartmentService = ({ deptId, lang }: any) => {
    ...
    const deleteService = async () => {
        ...
        try {
            ...
            // const item = sp.web.lists.getByTitle("Service_Task").items.getById(modal.form.id);
            ...
            setService(prev => {
                ...
                if (type) {
                    // Remove the item
                    newState[type].items = newState[type].items.filter((it: any) => it.id !== modal.form.id);
    ...
    const validateForm = () => {
        ...
        // image validation (ADD mode OR image removed in EDIT)
        if (
            modal.mode === "add" &&
            !modal.form.Icon
        ) {
        ...
        if (
            !modal.form.Icon &&     // no new upload
            !modal.form.iconUrl    // no existing image
        ) {
            newErrors.Icon = "Icon is required";
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (modal.mode === "add") {
                // await ServiceTaskService.addService(modal.form, deptId.id);
    ...
    const renderServices = () => {
        ...
        if (!Service || Object.keys(Service).length === 0) {
            return (
                ...
                //     <i className="fas fa-inbox"></i>
                //     <span>No Service and Tasks  found</span>
                // </div>
            );
        }
        ...
        return Object.values(Service).map((group: any) => (
            <div key={group.DeptType_En} className={styles.departmentSection}>
                ...
                }}>{lang ? group.DeptType_Ar : group.DeptType_En}</h3>
    ...
    const getServiedata = React.useCallback(async () => {
        setLoading(true);
        try {
            // const data = await ServiceTaskService.getServices(deptId.id);
        ...
        <div>
            <section className={styles.servicesSection}>
                <h2 className={` dds-section-title ${styles.ddsSectionTitle}`}>
                    ...
                    {isAdmin && <i className='fas fa-plus' onClick={openAddModal}></i>}
                ...
                <div className={styles.servicesContainer}>
                    {renderServices()}
                    {/* {Object.values(Service).map((group: any) => (
                        <div key={group.itemType} className={styles.departmentSection}>
                            <h3 >
                                {lang ? group.DeptType_Ar : group.DeptType_En}
                            </h3>
                            <div className={styles.serviceGrid}>
                                {group.items.map((item: any) => (
                                    <div key={item.id} className={styles.serviceCard} onClick={() => {
                                        if (isAdmin) {
                                            openEditModal(item)
                                        }
                    ...
                    <div className="form-flex-tiles ">
                        ...
                        <div className="form-row ">
                            ...
                            <CustomDropDown
                                ...
                                name="DeptType_Ar"
                                value={
                                    itemTypeMap.find(i => i.en === modal.form.DeptType_En)?.ar ?? null
                                }
