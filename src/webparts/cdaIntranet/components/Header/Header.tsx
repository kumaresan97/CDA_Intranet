...
const Header = () => {
  ...
  const buildDropdownData = (sectors: any[], departments: any[], lang: any) => {
    ...
    departments.forEach((dep) => {
      const sectorName = lang === "ar" ? dep.Sector.Title_Ar : dep.Sector.Title;
  ...
  const handleMobileDepartmentsToggle = () => {
    setDropdownState((prev) => ({
      ...prev,
      view: prev.view === "sectors" ? "departments" : "sectors",
    }));
  };
  ...
  const renderDesktopDropdown = () => (
    <div className={styles.dropdowncontainer}>
      ...
      >
        ...
        <div className={styles.sectorItem}>
          {sectors.map((sector) => (
            ...
            >
              <span className={styles.fontSemiBlue}>{sector}</span>
              <i
                className={`fas ${
      ...
      {/* Departments Column */}
      <div className={styles.departmentContainer} id="departments-pane">
        {hoveredSector && departmentsData[hoveredSector] ? (
          <>
            ...
            >
              ...
              >
                ...
                <i
                  className={
                    isArabic ? "fas fa-arrow-left" : "fas fa-arrow-right"
                  }
                  style={{
          ...
          <div className={styles.emptyMsg}>
            <i className="fas fa-mouse-pointer fa-2x mb-3" />
            <p>
              {isArabic
                ? "مرر الفأرة فوق قطاع لعرض الإدارات"
                : "Hover over a sector to view departments"}
            </p>
          </div>
        )}
      </div>
      ...
      return (
        <>
          <div className="flex items-center mb-4">
            ...
            >
              <i className="fas fa-arrow-right ml-2" /> [translate:رجوع]
            </button>
            <h3 className="flex-grow text-center font-bold text-white text-lg">
              {dropdownState.selectedSector}
            </h3>
          </div>
          <div className="space-y-1">
            {departments.map((dept) => (
              <a
                key={dept.name}
                href={dept.url}
                className="block text-white/90 p-2 rounded-lg hover:bg-white/20 text-sm pr-6"
              >
                {dept.name}
              </a>
            ))}
  ...
  return (
    <div>
      <header className={styles.header}>
        <div className={styles.container}>
          <div className={styles.topBar}>
            ...
            <div className={styles.logoRight}>
              <a target="_blank">
                <img src={logo1} className={styles.logoDubai} />
              </a>
            </div>
        ...
        <nav
          id="main-nav"
          className={`bg-dds-blue-700  ${styles.mainNavHeader}`}
        >
          <div className={styles.mainNavContainer}>
            <div className={`navbar-links ${styles.subMenu}`}>
              <div className={styles.leftSection}>
                ...
                >
                  <button
                    id="departments-dropdown-button"
      ...
      <div id="mobile-menu" className={styles.mobileSidebar}>
        <div className={styles.mobilemenuContainer}>
          ...
          <a className={styles.menuItem}>الرئيسية</a>
          ...
          <div>
            ...
            <div id="mobile-departments-content" className={styles.hiddenList}>
          ...
          <a className={styles.menuItem}>الإشعارات</a>
          <a className={styles.menuItem}>اتصل بنا</a>
        </div>
      </div>
    </div>
  );
