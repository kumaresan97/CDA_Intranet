...
const SectorLayout = ({ id, home }: any) => {
  ...
  const getSectorById = async (sectorId: number, isArabic: boolean) => {
    ...
    return {
      ...
      imgUrl:
        sector.AttachmentFiles.length > 0
          ? sector.AttachmentFiles[0].ServerRelativeUrl
          : null,
    };
  };
  ...
  const getDepartmentsBySector = async (sectorId: number) => {
    ...
    const departments = items.map((dept) => ({
      id: dept.ID,
      ...
      about: dept.About,
      imgUrl:
        dept.AttachmentFiles.length > 0
          ? dept.AttachmentFiles[0].ServerRelativeUrl
          : null,
      FileName:
        dept.AttachmentFiles.length > 0
          ? dept.AttachmentFiles[0].FileName
          : null,
      Icon: null,
    ...
    Modal.confirm({
      ...
      async onOk() {
        try {
          ...
          message.error(
            isArabic ? "حدث خطأ أثناء الحذف" : "Failed to delete department",
          );
        } finally {
          setDeptLoading(false);
        }
    ...
    return (
      <div className={styles.departmentsGrid}>
        {Array.from({ length: count }).map((_, index) => (
          <div key={index} className={`dds-card ${styles.departmentCard}`}>
            {/* Header */}
        ...
        <>
          {sectorLoading ? (
            ...
            <>
                ...
                {isArabic ? (
                  <ol className={styles.breadcrumbList}>
                    <li className={styles.breadcrumbItem}>
                      <a onClick={home} className={styles.breadcrumbLink}>
                        <i className={`${styles.homeIcon} fas fa-home`} />
                    ...
                    <li>
                      <div className={styles.breadcrumbCurrent}>
                        <i
                          className={`${styles.chevronIcon} fas fa-chevron-left`}
                        />
                  ...
                  <ol className={styles.breadcrumbList}>
                    <li>
                      <div className={styles.breadcrumbCurrent}>
                        ...
                        <i
                          className={`${styles.chevronIcon} fas fa-chevron-right`}
                        />
                      </div>
                    </li>
              ...
              <section className={styles.heroSection}>
                ...
                <h1 className={styles.heroTitle}>{sector?.title}</h1>

                <p className={styles.heroDescription}>{sector?.description}</p>
              </section>
              ...
              <section className={styles.aboutSection}>
                ...
                <p className={styles.sectionText}>{sector?.overview}</p>
              </section>
            </>
          )}
          ...
          {deptLoading ? (
            ...
            <div className="no-data">
              ...
              <p>
                {isArabic ? "لا توجد إدارات تابعة" : "No departments found"}
              </p>
            </div>
          )}
        </>
